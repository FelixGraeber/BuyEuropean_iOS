...
struct ResultsView: View {
    ...
    private var shareText: String {
        // Helper to get country name from alpha2 code
        func getCountryName(from code: String?) -> String {
            guard let code = code, !code.isEmpty else { return "an unknown country" }
            return Locale.current.localizedString(forRegionCode: code) ?? code
        }

        let productName = response.identifiedProductName ?? "This product"
        let companyName = response.identifiedCompany ?? "the company"
        // Use response.potentialAlternatives and response.identifiedProductName directly
        // Ensure identifiedProductName has a fallback for cases where it's nil or empty but still a "product" context
        ...
        case .europeanCountry, .europeanAlly:
            let countryName = getCountryName(from: response.identifiedCompanyHeadquarters) // Using identifiedCompanyHeadquarters as per model
            // If product name is meaningful, use it, otherwise focus on company.
            let determinedName = (response.identifiedProductName.isEmpty || response.identifiedProductName == "Unknown Product") ? companyName : currentProductName
            let companyInfo = response.identifiedCompany ?? "a European company"
            return "\(determinedName) is European! (Made by \(companyInfo) in \(countryName)). Found with BuyEuropean: \(appStoreLink)"

        case .europeanSceptic, .europeanAdversary, .neutral: // Considered "Not European" for share text
            if let alternatives = response.potentialAlternatives, !alternatives.isEmpty {
                let alternativeExample = alternatives.first?.productName ?? "European brands" // Using productName from EuropeanAlternative
                return "\(currentProductName) isn't European. BuyEuropean suggested alternatives like \(alternativeExample). Check it out: \(appStoreLink)"
            } else {
                return "Checking product origins with the BuyEuropean app. \(currentProductName) was analyzed. Find out more: \(appStoreLink)"
            }

        case .cat, .dog, .animal, .human, .unknown:
            // Generic message for non-product or unclassified items
            return "I used the BuyEuropean app to analyze an item. See what it can do for you: \(appStoreLink)"

        // No default needed if all Classification cases are handled.
        // If new cases are added to Classification, this switch will need updating.
        // Consider adding a default if Classification might expand frequently and a generic message is acceptable.
        // default:
        //     return "I used the BuyEuropean app to analyze an item. See what it can do for you: \(appStoreLink)"
        }
    }
